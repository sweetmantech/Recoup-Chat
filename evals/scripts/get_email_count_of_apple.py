...
yaml_file = os.path.join(REGISTRY_PATH, "evals/get_email_count_of_apple.yaml")
with open(yaml_file, "w") as yf:
    yf.write(YAML)
    
data_dir = os.path.join(REGISTRY_PATH, "data/get_email_count_of_apple")
...
for artist_id, email in mock_data:
    ...
    content = {
        "input": (
            ...
            f"Question: What is the email count for users with an Apple Music account?\n\n"
            f"{instruction_str}"
        ),
