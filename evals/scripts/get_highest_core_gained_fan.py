...
import string
import requests
from get_highest_score_gained_fan import get_highest_score_gained_fan

CURRENT_DIR = os.path.dirname(__file__)
REGISTRY_PATH = os.path.join(CURRENT_DIR, "../registry")

YAML = """
get_highest_score_gained_fan:
  id: get_highest_score_gained_fan.dev.v0
  ...
  metrics: [accuracy]
get_highest_score_gained_fan.dev.v0:
  class: evals.elsuite.modelgraded.classify:ModelBasedClassify
  args:
    samples_jsonl: get_highest_score_gained_fan/samples.jsonl
...
yaml_file = os.path.join(REGISTRY_PATH, "evals/get_highest_score_gained_fan.yaml")
with open(yaml_file, "w") as yf:
    yf.write(YAML)
    
data_dir = os.path.join(REGISTRY_PATH, "data/get_highest_score_gained_fan")
...
for artist_id, email in mock_data:
    ...
    streams_count = get_highest_score_gained_fan(email, artist_id)
    if context['data']:
